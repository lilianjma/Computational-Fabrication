...
class LSystem extends BaseLSystem {
 ...
 // Production rules
  ...
  public void iterate() {
    ...
    // [TODONE]: Implement the procedure for using the rules to replace characters in the current string, 
  ...
  public void drawLSystem(Turtle t) {
    ...
    // Scale the movement, if necessary, to help keep the image in frame when it gets too big
    if (scaleFactor != 0) {
    ...
    // [TODONE]: Loop through each character in the iteration string, and do turtle operations based on the character
    for (int i = 0; i < currentIteration.length(); i++) {
      Character c = currentIteration.charAt(i); 
      // [TODONE]: Implement different l-system vocabulary
      switch (c) {
        case 'F':
          t.forward(dist);
